...
```powershell
# deps download OpenCL headers (+ Win import lib); then build + test
make            # = make deps && make build
make test       # laplaceOcl + csrOcl smokes
make run-laplace
make run-csr
...
Third-party OpenCL bits are **not** vendored: `make deps` pulls Khronos headers into `deps/` (gitignored).
...
deps/                  # generated by `make deps` (gitignored)
third_party/           # README only — no vendored code
```
